...
unsigned char calc_direction_bits(const float* destination )
{
  unsigned char direction_bits = 0;
  if (destination[X_AXIS] < planner.get_position(X_AXIS)) { 
    direction_bits |= (1<<X_AXIS); 
  }
  if (destination[Y_AXIS] < planner.get_position(Y_AXIS)) { 
    direction_bits |= (1<<Y_AXIS); 
  }
  if (destination[Z_AXIS] < planner.get_position(Z_AXIS)) { 
    direction_bits |= (1<<Z_AXIS); 
  }
  if (destination[E_AXIS] < planner.get_position(E_AXIS)) { 
    direction_bits |= (1<<E_AXIS); 
  }
  return direction_bits;
}

unsigned char calc_move_bits( const float* destination )
{
...
{
  float destination[NUM_AXIS] = {x*planner.axis_steps_per_mm[X_AXIS],y*planner.axis_steps_per_mm[Y_AXIS],z*planner.axis_steps_per_mm[Z_AXIS],e*planner.axis_steps_per_mm[E_AXIS]};
  unsigned char direction_bits = calc_direction_bits(destination );
  unsigned char move_bits = calc_move_bits(destination);
  ...
  if( (direction_change_bits & m_hysteresis_bits) != 0 )
  {
    ...
    for(int axis=0;axis<NUM_AXIS;++axis)
    {
      ...
      if( direction_change_bits & (1<<axis) )
      {
        ...
        planner.set_axis_position(axis, planner.get_position(axis) - fix);
        axis_shift[axis] += fix;
      }
    }
  }
